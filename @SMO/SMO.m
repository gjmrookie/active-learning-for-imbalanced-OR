...
%             end
        ...
        function k = Kernel(x, y)%the kernel function
            ...
            global KTYPE
            global KSCALE or_class_number
            
            %x devide into x_i and ek k = x_i*y_i' + ek*ek'
            x_i = x(:,1:end - or_class_number + 1);
            ek_x = x(:,end - or_class_number + 2:end);
            y_i = y(:,1:end - or_class_number + 1);
            ek_y = y(:,end-or_class_number + 2:end);
            k = x_i*y_i';
            if KTYPE == 1				% linear
                % take as is
